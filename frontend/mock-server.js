// Lightweight mock server for demo offline. Run with `node mock-server.js` from frontend/.
...
const server = http.createServer(async (req, res) => {
  ...
  if (parts[0] === 'api' && parts[1] === 'mock' && parts[2] === 'users') {
    // ensure file exists
    if (!fs.existsSync(FILES.users)) fs.writeFileSync(FILES.users, JSON.stringify([], null, 2));
    ...
    if (req.method === 'DELETE' && parts.length === 4) {
      ...
      fs.writeFileSync(FILES.users, JSON.stringify(remaining, null, 2));
      res.writeHead(200, { 'Content-Type': 'application/json' });
