...
async function getApp() {
  // dynamically import ESM server.js
  const mod = await import(pathToFileURL(path.join(__dirname, '..', 'server.js')).href);
...
describe('E2E: upload fallback + submission flow (cjs compat)', () => {
  ...
  test('upload fallback and submit', async () => {
    ...
    if (body.fallback) {
      ...
      expect([200,201]).toContain(upRes.status);
